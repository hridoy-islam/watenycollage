...
export function SideNav() {
  ...
  const isEmployeeTeacher =
    user?.role === 'employee' && isTeacherDesignation;
  const effectiveRole = isEmployeeTeacher ? 'teacher' : user?.role;

  const navItems = [
    ...(isCompleted ? [{ path: '/dashboard', label: 'Dashboard', icon: LayoutDashboard }] : []),
    ...(effectiveRole === 'admin' ? adminLinks : effectiveRole === 'teacher' ? teacherLinks : (effectiveRole === 'student' || effectiveRole === 'applicant') ? studentLinks : []),
  ];
