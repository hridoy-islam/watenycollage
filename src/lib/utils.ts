...
export const isAcademicInfoComplete = (student) => {
  ...
  const hasAcademicHistory = student.academicHistoryRequired || 
    (Array.isArray(student.academicHistory) && student.academicHistory.length > 0);

  // Check if English language exam is required or if there's valid data in the array
  const hasEnglishLanguageExam = student.englishLanguageRequired || 
    (Array.isArray(student.englishLanguageExam) && student.englishLanguageExam.length > 0);
