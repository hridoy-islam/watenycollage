...
// 1. UPDATED INTERFACE: ecertId can be an object (from API) or string (from local optimistic update)
...
export default function TrainingCertificatesPage() {
  ...
  // Fetch ecert types and user's uploaded certs
  useEffect(() => {
    if (!id) return;
  ...
  // 2. UPDATED MAPPING LOGIC: Extract the ID string correctly
  const uploadedMap = new Map<string, string[]>();

  uploadedCerts.forEach((item) => {
    // Check if ecertId is an object (API response) or string (local update)
    ...
    uploadedMap.get(ecertIdString)?.push(item.document);
  });

  // Filter uploadable options: only show ecerts NOT already uploaded
  ...
  const handleSubmitDocument = async () => {
    ...
    try {
      await axiosInstance.post('/ecert-form', {
        userId: id,
      ...
      const newUploadedCert: UploadedEcert = {
        ...
        ecertId: selectedEcert.value, // This is a string here
        document: uploadedFileUrl
      };
      ...
      // Check remaining
      const remainingOptions = ecertOptions
        .map((opt) => opt._id)
        .filter(
          (optId) => !uploadedMap.has(optId) && optId !== selectedEcert.value
        );
      ...
      setUploadError('Failed to link certificate. Please retry.');
    }
  };
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                  <DialogTrigger asChild>
                    <Button
                      ...
                      disabled={uploadableOptions.length === 0}
                    >
                    ...
                    <div className="space-y-4 py-4">
                      <div>
                        ...
                        <Select
                          ...
                          isDisabled={uploadableOptions.length === 0}
                        />
                      </div>
            ...
            {hasUploaded ? (
              <div className="space-y-6">
                {ecertOptions.map((ecert) => {
                  // Map lookup relies on the string ID we set up in the logic above
