...
function SpotCheckTab() {
  ...
  const submitSchedule = async () => {
    ...
    try {
      ...
      toast({ title: 'Spot check scheduled successfully', className: 'bg-watney text-white' });
  ...
  const submitCompletion = async () => {
    ...
    if (!inputDate || uploadedFiles.length === 0 || !spotCheckId) return;
    setIsSubmitting(true);
    ...
    try {
      ...
      await fetchSpotCheckData(); 
      toast({ title: 'Spot check marked as completed', className: 'bg-watney text-white' });
