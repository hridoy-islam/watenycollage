...
function ApplicantMailPage() {
  ...
  const fetchEmailLogs = async (page, entriesPerPage, searchTerm = '') => {
    ...
    setLoadingLogs(true);
    try {
      const res = await axiosInstance.get(`/email?userId=${userId}`, {
        params: {
          page,
  ...
  const replaceVariables = async (text: string, studentData: any) => {
    ...
    if (id) {
      try {
        ...
        const applicationTitle = data?.jobId?.jobTitle || '';
...
export default ApplicantMailPage;
