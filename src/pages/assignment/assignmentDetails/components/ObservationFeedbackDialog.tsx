...
interface ObservationFeedbackData {
  learningOutcomes: {
...
interface ObservationFeedbackDialogProps {
  isOpen: boolean;
  ...
  assignmentId: string;
  onSubmit: (feedbackData: ObservationFeedbackData) => Promise<void>;
  initialData?: ObservationFeedbackData;
  isSubmitting?: boolean;
  isEditing?: boolean; // Add this new prop
}

export const ObservationFeedbackDialog: React.FC<ObservationFeedbackDialogProps> = ({
  isOpen,
  onOpenChange,
  unitMaterial,
  assignmentId,
  ...
  const [feedbackData, setFeedbackData] = useState<ObservationFeedbackData>({
    learningOutcomes: []
  });
...
//       const baseData = prev?.learningOutcomes?.length
//         ? prev
//         : initialData; // fallback if prev empty

//        const mergedLearningOutcomes = unitMaterial.learningOutcomes
//           .filter((lo) => lo.observation === true)
//           .map((lo) => {
...
// }, [isOpen, unitMaterial, initialData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title:
          'Observation feedback submitted successfully!'
      });
      onOpenChange(false);
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to submit observation feedback.',
        variant: 'destructive'
      });
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95%] max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">{isEditing ? 'Edit Observation Feedback' : 'Observation Feedback Assessment'}</DialogTitle>
        </DialogHeader>
