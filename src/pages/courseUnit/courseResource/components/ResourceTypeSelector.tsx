...
const ResourceTypeSelector: React.FC<ResourceTypeSelectorProps> = ({
  ...
  const resourceTypes = [
    {
      ...
      icon: GraduationCap,
      title: 'Introduction',
      description: 'Create a course introduction with welcome message and overview',
      bgColor: 'bg-blue-100',
  ...
  return (
    <div className="grid grid-cols-1 gap-6 py-6 md:grid-cols-4">
      {resourceTypes.map(({ type, icon: Icon, title, description, bgColor, iconColor, hoverColor, disabled }) => (
        <Card
          key={type}
