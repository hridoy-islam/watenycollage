...
// --- Main Component ---
export default function EditPostEmploymentMedicalForm() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [medicalFormId, setMedicalFormId] = useState<string | null>(null); // ✅ Added state to store form ID
  
  // Note: user and isSubmitted were unused in original, keeping them if needed for other logic
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 1. Try to fetch existing medical form data
        let medicalData = null;
        try {
          const res = await axiosInstance.get(`/medical-form?userId=${id}`);
          if (res.data && res.data.data) {
             medicalData = Array.isArray(res.data.data.result) 
                ? res.data.data.result[0] // handle case where it returns array
                : res.data.data.result;
          }
        } catch (err) {
           // Ignore 404 or fetch error, proceed to fetch defaults from user profile
           console.log("No existing medical form found, fetching user details.");
        }

        if (medicalData) {
          // ✅ Capture the form ID for the PATCH request later
          setMedicalFormId(medicalData._id); 

          form.reset({
            ...medicalData,
            dateOfBirth: medicalData.dateOfBirth
              ? new Date(medicalData.dateOfBirth)
              : undefined,
          });
        } else {
          // No existing form - Pre-fill from User Profile
        ...
        // ✅ Fixed toast error accessing
        toast({ 
          title: err?.response?.data?.message || 'Failed to load form data. Please try again.', 
          variant: 'destructive'
        });
        setLoading(false);
      }
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    // Safety check if we are in "Edit" mode but haven't loaded an ID
    if (!medicalFormId) {
        toast({ title: "Error: Could not determine which form to update.", variant: "destructive" });
        return;
    }
    ...
    try {
      ...
      toast({ title: "Medical form updated successfully."});
      navigate(-1);
    } catch (err: any) {
      toast({ 
        title: err?.response?.data?.message || 'Failed to submit form. Please try again.', 
        variant: 'destructive'
      });
    } finally {
      setSubmitting(false);
