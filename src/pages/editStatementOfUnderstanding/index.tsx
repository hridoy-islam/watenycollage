...
export default function EditStatementOfUnderstanding() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let existingData: { _id: string; signatureUrl?: string } | null = null;
      try {
        const dataRes = await axiosInstance.get(`/statement-of-understanding?userId=${id}`);
        ...
        console.log('No existing statement of understanding form found.');
      }
  ...
  const onSubmit = async (data: StatementFormValues) => {
    ...
    try {
      ...
      if (formId) {
        await axiosInstance.patch(`/statement-of-understanding/${formId}`, payload);
        toast({
          title: 'Success',
          description: 'Statement of Understanding updated successfully.',
        });
      } else {
        await axiosInstance.post('/statement-of-understanding', payload);
        toast({
          title: 'Success',
          description: 'Statement of Understanding submitted successfully.',
        });
      }
      ...
      toast({
        ...
        description: error?.response?.data?.message || 'Could not save statement of understanding form.',
        variant: 'destructive',
      });
    } finally {
      setSubmitting(false);
      ...
      <div className="w-full">
        <Card className="overflow-hidden border-0 shadow-xl">
          <CardHeader className="border-b border-gray-100 bg-white pb-6 pt-8">
            ...
            <CardDescription>
              Edit the statement of understanding form details.
            </CardDescription>
          </CardHeader>
