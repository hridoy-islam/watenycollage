...
export default function InvoicesPage() {
  ...
  const handleConfirmMarkAsPaid = async () => {
    ...
    try {
      await axiosInstance.patch(`/invoice/${invoiceToMark}`, { status: "paid" });

      setInvoices((prevInvoices) =>
        prevInvoices.map((invoice) =>
          invoice._id === invoiceToMark ? { ...invoice, status: "paid" } : invoice
        )
      );
  ...
  const handleConfirmExport = async () => {
    ...
    try {
      const invoiceResponse = await axiosInstance.get(`/invoice/${invoiceToExport}`);
      ...
      const payload = {
        ...
        description: invoiceData.students.map((student: any) => student.refId).join(", "),
      ...
      await axiosInstance.patch(`/invoice/${invoiceToExport}`, { exported: true });
    ...
    <div className="mx-auto py-1">
      <div className="flex justify-between">
        ...
        <div className="space-x-4">
          ...
          <Link to="status">
            <Button className="bg-supperagent text-white hover:bg-supperagent">Check Status</Button>
          </Link>
          <Link to="remit">
            <Button className="bg-supperagent text-white hover:bg-supperagent">Remit List</Button>
          </Link>
  ...
  <div className="flex space-x-6">
    ...
    <div className="flex items-center mt-7">
      <Button
        className="bg-supperagent text-white hover:bg-supperagent min-w-[120px]"
              ...
              {invoices.length > 0 ? (
                invoices.map((invoice) => (
                  <TableRow key={invoice._id}>
                    ...
                    <TableCell>{invoice.status === "due" ? (
                      <div className="flex flex-row items-center justify-start gap-2">
                        <span className="text-xs text-red-500 font-semibold">Due</span>
                        <Button
                          size="sm"
                          className="bg-supperagent text-white hover:bg-supperagent max-w-[100px]"
                          onClick={() => {
                      ...
                      <span className="text-green-500 font-semibold text-xs">Paid</span>
                    )}</TableCell>
                    <TableCell>
