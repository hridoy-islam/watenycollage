...
export default function CharacterReferencePage() {
  ...
  useEffect(() => {
    const checkRefStatus = async () => {
      try {
        ...
        const res = await axiosInstance.get(`/users/${id}?fields=ref3Submit`);
        const user = res.data.data
        // Check which ref param is being used and update isUsed
        if (ref === "ref3" && user.ref3Submit === true) {
          setIsUsed(true);
        } else {
          setIsUsed(false);
        }
  ...
  const onSubmit = async (data: CharacterReferenceFormData) => {
    try {
      ...
      if (ref === "ref3") {
        refFlagPayload.ref3Submit = true;
      }
