  ...
  return (
    ...
    >
      <div className="space-y-8">
        ...
        <div>
          <h3 className="mb-4 text-lg font-medium text-gray-900">English Qualification</h3>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {[
              { key: 'englishTestType', label: 'Test Type' },
              { key: 'englishTestScore', label: 'Test Score' },
              { key: 'englishTestDate', label: 'Test Date', isDate: true },
              { key: 'englishCertificate', label: 'Certificate' }
              ...
              <div key={key}>
                ...
                {isEditing ? (
                  isDate ? (
                    <CustomDatePicker
                      selected={
                        localData?.englishQualification?.[key] ? new Date(localData.englishQualification[key]) : null
                      }
                      onChange={(date) =>
                        setLocalData((prev) => ({
                          ...prev,
                          englishQualification: {
                            ...prev.englishQualification,
                            [key]: date
                          }
                        }))
                      }
                    ...
                    <Input
                      value={localData?.englishQualification?.[key] || ''}
                      onChange={(e) =>
                        setLocalData((prev) => ({
                          ...prev,
                          englishQualification: {
                  ...
                  <div className="mt-1 text-gray-900">
                    {isDate && localData?.englishQualification?.[key]
                      ? new Date(localData.englishQualification[key]).toLocaleDateString()
                      : localData?.englishQualification?.[key] || '-'}
                  </div>
                )}
              </div>
            ))}
