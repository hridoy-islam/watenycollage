...
export default function RemitCreatePage() {
  ...
  const fetchStudentsForInvoice = async (courseRelationId, year, session) => {
    try {
      ...
      const relationResponse = await axiosInstance.get(
        `/course-relations/${courseRelationId._id}`
      );
      ...
      if (relationData) {
        ...
        const invoiceStudentsResponse = await axiosInstance.get(
          `/invoice/${id}`
        );
        const invoiceStudents =
          invoiceStudentsResponse?.data?.data.students || [];
        ...
        if (!sessionObj) {
          console.error(
            'Session details not found for year:',
            year,
            'session:',
            session
          );
        ...
        const selectedIds = new Set(
          selectedStudentsWithFees.map((s) => s.refId)
        );
  ...
  const fetchStudents = async (filters: z.infer<typeof filterSchema>) => {
    try {
      ...
      if (term && course && institute) {
        ...
        if (!matchingRelation) {
          console.warn('No matching course relation found for filters:', {
            term,
            course,
            institute
          });
    ...
    if (!isAlreadySelected) {
      ...
      if (!application) {
        toast({
          title: 'Error',
          description:
            'Application not found for this student and course relation.',
          variant: 'destructive'
        });
        return;
      }
  ...
  const handleUpdateInvoice = async () => {
    ...
    try {
      ...
      const calculatedTotalAmount = formattedStudents.reduce(
        ...
        <Card>
          <div className="p-4">
            ...
            <select
              ...
              className="min-w-[250px] rounded-md border border-gray-300 p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
            ...
            {form.formState.errors.remit && (
              <p className="mt-1 text-sm text-red-500">
                {form.formState.errors.remit.message}
              </p>
            )}
          </div>
