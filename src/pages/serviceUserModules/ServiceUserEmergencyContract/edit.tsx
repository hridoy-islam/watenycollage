...
export default function EditEmergencyContractPage() {
  const { sid, id } = useParams<{ sid: string; id: string }>();
  ...
  useEffect(() => {
    ...
    const fetchContract = async () => {
      try {
        setIsLoading(true);
        const res = await axiosInstance.get(`/serviceuser-emergency-contract/${id}`);
        const item = res.data?.data;
        if (item) {
          setSelectedType(item.contractType as ContractType);
          setFormData({
            ...
            isUnpaidCarer: item.isUnpaidCarer || false,
            isNextOfKin: item.isNextOfKin || false,
            isDependant: item.isDependant || false,

            isProxyForSubjectOfCare: item.isProxyForSubjectOfCare || false,
            lpaHealthAndWellbeing: item.lpaHealthAndWellbeing || false,
            lpaFinancial: item.lpaFinancial || false,
            lpaDateAppointed: item.lpaDateAppointed ? new Date(item.lpaDateAppointed) : null,
            lpaFormLocation: item.lpaFormLocation || '',

            postcode: item.postcode || '',
            address: item.address || '',
            telephone1: item.telephone1 || item.phoneNumber || '',
            telephone2: item.telephone2 || item.alternatePhoneNumber || '',
            email: item.email || '',
            contactStatusPriority: item.contactStatus === 'Priority',
            contactStatusSecondary: item.contactStatus === 'Secondary',
            contactStatusDoNotContact: item.contactStatus === 'Do not contact',
            contactableTimes: item.contactableTimes || '',

            familyAppAccessYes: item.familyAppAccess === true,
            familyAppAccessNo: item.familyAppAccess === false,
            familyAppAccessJustification: item.familyAppAccessJustification || '',

            gpPracticeCode: item.gpPracticeCode || '',
            gpPracticeName: item.gpPracticeName || '',
            gpBuildingName: item.gpBuildingName || '',
            gpAddressLine1: item.gpAddressLine1 || '',
            gpAddressLine2: item.gpAddressLine2 || '',
            gpTownCity: item.gpTownCity || '',
            gpCountry: item.gpCountry || '',
            gpPostcode: item.gpPostcode || '',
            gpPhoneNumber: item.gpPhoneNumber || '',
            gpEmail: item.gpEmail || '',

            professionalType: item.professionalType || '',
            professionalRegistrationNumber: item.professionalRegistrationNumber || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axiosInstance.patch(`/serviceuser-emergency-contract/${id}`, payload);
      navigate(-1);
    } catch (error) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <CardDescription className="mt-1 text-black">
                  Update the details for the {selectedType?.toLowerCase()}{' '}
                  emergency contact
                </CardDescription>
              </div>
            </div>
